...
const ContactLink = ({ icon: Icon, label, value, href, delay = 0 }) => (
  <motion.a
    ...
    viewport={{ once: true }}
    className="group relative flex items-start gap-4 p-6 rounded-xl bg-surface-light border border-purple-500/10
               hover:border-purple-500/30 transition-all duration-300"
  >
    <div
      className="relative p-3 rounded-full bg-surface-dark text-purple-400 group-hover:scale-110 
...
const SocialLink = ({ link, index }) => (
  ...
  >
    <motion.a
      ...
      className="relative block p-4 rounded-full bg-surface-light border border-purple-500/10
                hover:border-purple-500/30 transition-all duration-300 group z-10"
      whileHover={{ scale: 1.1 }}
      whileTap={{ scale: 0.95 }}
    >
      <link.icon className="w-6 h-6 text-gray-400 group-hover:text-purple-400 transition-colors duration-300" />

      {/* Improved tooltip positioning and visibility */}
      <span
        className="absolute -bottom-10 left-1/2 -translate-x-1/2 px-2 py-1 text-sm 
                    bg-surface-dark border border-purple-500/10 rounded-md
  ...
  const contactInfo = [
    {
      icon: FaPhone,
      label: "Phone",
      value: "+880 1771 769898",
      href: "tel:+8801771769898",
    },
    {
      icon: FaEnvelope,
      label: "Email",
      value: "ayon7930@gmai.com",
      href: "mailto:ayon7930@gmai.com",
    },
    {
      icon: FaMapMarkerAlt,
      label: "Location",
    ...
    >
      ...
      <div className="container mx-auto px-6 relative">
        ...
        >
          <motion.a
            href=""
            className="inline-flex items-center gap-3 px-8 py-4 bg-purple-500 text-surface-dark 
                     font-medium rounded-full hover:bg-purple-400 transition-colors duration-300
